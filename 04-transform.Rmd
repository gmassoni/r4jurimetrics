...
```{r, echo=TRUE}
d_2014 <- readRDS("data-raw/espec/d_2014.rds")
...
```{r, echo=TRUE}
arqs <- dir("data-raw/espec/", 
            pattern = "[0-9]{4}",
            full.names = TRUE)

df <- map_dfr(arqs, file_to_cpo)
df %>% View
...
summarise_dt <- function(dt, txt, re, .fun, ignore_case = TRUE) {
  ...
  my_regex <-  regex(re, ignore_case = ignore_case)
  tiblle(dt = dt, txt = txt) %>% 
    filter(str_detect(txt, my_regex)) %>% 
    summarise(resumo = .fun(dt)) %>% 
    pull(resumo)
}
```
...
summarise_movs <- function(m) {
  ...
  partial_summarise_mov <- partial(summarise_dt,
                                   dt = dmy(m$data_mov),
                                   txt = m$mov)
...
d_espec <- readRDS("data-raw/espec/d_espec.rds")
