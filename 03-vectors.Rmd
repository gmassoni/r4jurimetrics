...
c("2017-04-10", "2017-08-08", "2017-02-28") %>% 
  ymd() %>% 
  ceiling_date("half year") #arredonda pra frente o SEMESTRE
```
...
str_replace(string, 'parcialmente ', '') #substitui só o primeiro
str_replace_all(string, 'parcialmente ', 'x ') #substitui todos (n) por n padrões
str_replace(string, '(parcialmente )+', 'x ')  #substitui padrões repetidos por apenas um padrão
...
str_match(frases, 'o (ro)') #parenteses coloca resultados em outra coluna
```

- `str_split()` separa uma string em várias de acordo com um separador.

```{r echo=TRUE}
string <- c('eu, sei','eu sei, usar virgulas, de forma, perfeita')

str_split(string, ', ') #retorna uma lista de vetores
str_split(string, ', ', simplify = TRUE) #retorna uma matriz em que cada linha representa uma posição do vetor de entrada (string)
...
```{r echo=TRUE}
negaram <- regex('negara?m|mantiv|negam|nao conhec', ignore_case = TRUE)
parcial <- regex('parcial|em part', ignore_case = TRUE)
deram <- regex('deram|nulidade|anul', ignore_case = TRUE)
extinto <- regex('extin', ignore_case = TRUE) 


tipos_decisao <- function(decisoes) {
  decisoes <- rslp:::remove_accents(decisoes)
  case_when(
...
```{r echo=TRUE}
str_trunc(d_decisoes$decision[tipo_decisao == "outros"], 1500) %>% 
  head(10)

str_detect(d_decisoes$decision[tipo_decisao == "outros"][2], parcial)
```
...
```{r}
d_cjsg %>% 
  select(ends_with('ion'), contains("sum"))
...
```{r}
d_cjsg %>% 
  select(-contains("sum")) #As colunas que contém texto são as sum
...
```{r}
d_cjsg %>% 
  filter( year(dmy(dt_decision)) == 2018)
```
...
3. A aplicação de `filter` modifica o objeto `d_cjsg`? Resposta: Não
...
```{r}
d_cjsg %>% 
  mutate(comarca_limpa = rslp:::remove_accents(district))
```
...
```{r}
d_cjsg %>% 
  mutate(drogas = str_detect(summary, "droga"))
```
...
```{r}
d_cjsg %>%
  select(rapporteur, summary) %>%
  mutate(tam_sent = str_length(summary)) %>%
  group_by(rapporteur) %>%
  summarise(n = n(),
            tam_sent_media = mean(tam_sent)) %>%
  filter(n > 10) %>%
  arrange(desc(tam_sent_media))
```
